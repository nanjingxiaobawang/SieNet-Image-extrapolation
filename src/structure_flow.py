        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # flow model
                    ...
                    logs = self.flow_model.update_flow(inputs,None, smooths, gts, maps, self.flow_model.use_correction_loss, self.flow_model.use_vgg_loss,mode=2)
                    iterations = self.flow_model.iterations
                # flow with structure model
                elif model == 3:
                    with torch.no_grad(): 
                        smooth_stage_1 = self.flow_model.structure_forward(inputs, smooths, maps)
                        smooth_gt = self.flow_model.structure_forward(gts, smooths, torch.zeros_like((maps)))
                    logs = self.flow_model.update_flow(inputs, smooth_gt.detach(),smooth_stage_1.detach(), gts, maps, self.flow_model.use_correction_loss, self.flow_model.use_vgg_loss,mode=3)
                    iterations = self.flow_model.iterations
        ...
        with torch.no_grad(): 
            ...
            for j, items in enumerate(islice(val_loader, 50)):
                ...
                # inpaint model
                    ...
                    outputs, flow_maps,_ = self.flow_model.flow_forward(inputs, smooths, maps)
                ...
                # inpaint with structure model
                    ...
                    smooth_stage_1 = self.flow_model.structure_forward(inputs, smooths, maps)
                    outputs, flow_maps, _ = self.flow_model.flow_forward(inputs, smooth_stage_1, maps)
                    psnr, ssim, l1 = self.metrics(outputs, gts)
                    logs.append(('psnr', psnr.item()))
                    logs.append(('SSIM', ssim.item()))
                    psnr_list.append(psnr.item())
