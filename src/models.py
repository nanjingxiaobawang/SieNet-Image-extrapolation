...
class StructureFlowModel(BaseModel):
    ...
    def flow_forward(self, inputs, stage_1, maps):
        outputs, flow, cross_simeans = self.f_gen(torch.cat((inputs, stage_1, maps),dim=1))
        return outputs, flow, cross_simeans
    ...
    def sample(self, inputs, smooths, gts, maps):
        with torch.no_grad(): 
            if self.config.MODEL == 1:
                ...
                outputs, flow,_ = self.flow_forward(inputs, smooths, maps)
                result =[inputs,smooths,gts,maps,outputs]
                if flow is not None:
                ...
                outputs, flow, _ = self.flow_forward(inputs, smooth_stage_1, maps)
    ...
    def update_flow(self, inputs,gt_input, smooths, gts, maps, use_correction_loss, use_vgg_loss,mode):
        self.iterations += 1

        self.f_dis.zero_grad()
        self.f_gen.zero_grad()
        if mode != 3 and gt_input != None:
            raise NotImplemented

        if mode == 3:
            outputs, flow_maps, cross_simeas_1 = self.flow_forward(inputs, smooths, maps)
            output_gt, _,  cross_simeas_2= self.flow_forward(gt_input,smooths,torch.zeros_like(maps))
        else:

            outputs, flow_maps, _ = self.flow_forward(inputs, smooths, maps)
        ...
        if mode == 3:
            self.simean_loss = self.l2_loss(cross_simeas_1,cross_simeas_2)
            self.flow_loss = self.flow_adv_gen_loss + self.flow_l1_loss + self.flow_correctness_loss + self.vgg_loss + self.simean_loss
        if mode==2:
        ...
        if mode != 3:
            ...
            logs = [
                ...
                ("l_f_l1_gen", self.flow_l1_loss.item()),
                ("l_f_total_gen", self.flow_loss.item()),
                ("l_simeans",self.simean_loss.item())
            ]
