...
from fillingconv import FillingConv2d
...
class StructureGen(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_res=4, activ='relu', 
        ...
        #用到了spertal normlization
        self.down_sample=nn.ModuleList()
        self.up_sample=nn.ModuleList()
        self.content_param=nn.ModuleList()

        self.input_layer = Conv2dBlock(input_dim*2+1, dim, 7, 1, 3, norm, activ, pad_type, use_sn=use_sn) #shape=(4,64,256,512)
        self.down_sample += [nn.Sequential(
            Conv2dBlock(dim, 2*dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn),#shape=(4,128,128,256)
            Conv2dBlock(2*dim, 2*dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn))]#shape=(4,128,128,256)

        self.down_sample += [nn.Sequential(
            Conv2dBlock(2*dim, 4*dim, 4, 2, 1,norm, activ, pad_type, use_sn=use_sn),#shape=(4,256,64,128)
            Conv2dBlock(4*dim, 4*dim, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn))]#shape=(4,256,64,128)

        self.down_sample += [nn.Sequential(
            Conv2dBlock(4*dim, 8*dim, 4, 2, 1,norm, activ, pad_type, use_sn=use_sn))]#shape=(4,512,32,64)
        ...
        self.up_sample += [(nn.Sequential(
            ...
            Conv2dBlock(dim, dim // 2, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)) )]# output_dim=256 pad_type=reflect SN,shape=(4,256,64,128)

        self.up_sample += [(nn.Sequential(
            ResBlocks(n_res, dim//2, norm, activ, pad_type=pad_type),
            nn.Upsample(scale_factor=2),
            Conv2dBlock(dim//2, dim//4, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn)) )] #shape=(4,128,128,256)

        self.up_sample += [(nn.Sequential(
            ResBlocks(n_res, dim//4, norm, activ, pad_type=pad_type),
            nn.Upsample(scale_factor=2),
            Conv2dBlock(dim//4, dim//8, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn)) )]  #shape=(4,64,256,512)

        self.content_param += [Conv2dBlock(dim//2, dim//2, 5, 1, 2, norm, activ, pad_type)]#shape=(4,256,256,512)
        self.content_param += [Conv2dBlock(dim//4, dim//4, 5, 1, 2, norm, activ, pad_type)]#shape=(4,128,256,512)
        self.content_param += [Conv2dBlock(dim//8, dim//8, 5, 1, 2, norm, activ, pad_type)]#shape=(4,64,256,512)
        self.image_net = Get_image(dim//8, input_dim)
        #shape=(4,3,256,512)
        self.sa=Self_Attn(512,'relu')
        self.sc=CAM_Module(512,'relu')



    def forward(self, inputs):
        x0 = self.input_layer(inputs)#shape=(4,64,256,512)
        x1 = self.down_sample[0](x0)#shape=(4,128,128,256)
        x2 = self.down_sample[1](x1)#shape=(4,256,64,128)
        x3 = self.down_sample[2](x2)#shape=(4,512,32,64)
        #attention
        #SE_net1=self.se(x3)
        # sa_feat,p1 = self.sa(x3)
        #
        # sc_feat,p2 = self.sc(x3)
        #
        # feat_sum = sa_feat + sc_feat
        ## sasc_output =self.down_sample[2](feat_sum)
        ## output = [sasc_output]
        ## output.append(sa_output)
        ## output.append(sc_output)

        u1 = self.up_sample[0](x3) + self.content_param[0](x2)#shape=(4,256,64,128)+shape=(4,256,64,128)=(4,256,64,128)=u1
        u2 = self.up_sample[1](u1) + self.content_param[1](x1)#shape=(4,128,128,256)
        u3 = self.up_sample[2](u2) + self.content_param[2](x0)#shape=(4,64,256,512)
...
class FlowGen(nn.Module):
    ...
    def forward(self, inputs):
        ...
        images_out, cross_simeans = self.conv_column(inputs, flow_map)
        return images_out, flow_map , cross_simeans
...
class ConvColumn(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_res=2, activ='lrelu',
        ...
        self.down_sample += [nn.Sequential(
                            FillingConv2d(in_channels=7, out_channels=7, kernel_size=7, stride=1, padding=1, bias=False),
                            Conv2dBlock(input_dim*2+1, dim//2, 7, 1, 3, norm, activ, use_sn=use_sn),#shape=(4,32,256,512)


                            
                            Conv2dBlock(dim//2, dim,   4, 2, 1, norm, activ, pad_type,use_sn=use_sn),        #shape=(4,64,128,256)

                            
                            Conv2dBlock(dim,    dim,   5, 1, 2, norm, activ,pad_type, use_sn=use_sn),        #shape=(4,64,256,512)
        ...
        self.down_sample += [nn.Sequential(
                            Conv2dBlock(2*dim,  4*dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn),        #shape=(4,256,128,256)
                            Conv2dBlock(4*dim,  8*dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn))]       #shape=(4,512,64,128)
        dim = 8*dim
        #self.se = SEModule(dim, reduction=4)
        # content decoder
        self.up_sample   += [(nn.Sequential(
                             ResBlocks(n_res, dim, norm, activ, pad_type=pad_type),
                             nn.Upsample(scale_factor=2),                                                      #shape=(4,512,128,256)
                             Conv2dBlock(dim, dim//2, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)) )]       #shape=(4,256,128,256)
        self.up_sample   += [(nn.Sequential(
                            Conv2dBlock(dim, dim//2, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn),            #shape=(4,256,128,256)
                            ResBlocks(n_res, dim//2, norm, activ, pad_type=pad_type),
                            nn.Upsample(scale_factor=2),                                                       #shape=(4,256,256,512)
                            Conv2dBlock(dim//2, dim//4, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn),         #shape=(4,128,256,512)

                            ResBlocks(n_res, dim//4, norm, activ, pad_type=pad_type),
                            nn.Upsample(scale_factor=2),                                                       #shape=(4,128,256,512)
                            Conv2dBlock(dim//4, dim//8, 5, 1, 2,norm, activ, pad_type, use_sn=use_sn),         #shape=(4,64,256,512)
                            Get_image(dim//8, input_dim)) )]                                                   #shape=(4,3,256,512)
    ...
    def forward(self, inputs, flow_maps):
        ...
        return images_out, cross_simeans

    # def forward(self, inputs):
    #     x1 = self.down_sample[0](inputs)
    #     x2 = self.down_sample[1](x1)
    #
    #     images_out = self.up_sample[1](x2)
    #     return images_out
...
class FlowColumn(nn.Module):
    def __init__(self, input_dim=3, dim=64, n_res=2, activ='lrelu',
        ...
        self.down_sample_flow.append( nn.Sequential(
                                    Conv2dBlock(input_dim*2+1, dim//2, 7, 1, 3, norm, activ, pad_type, use_sn=use_sn),   # shape=(4,32,256,512)
                                    Conv2dBlock(dim//2, dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn),             # shape=(4,64,128,256)
                                    Conv2dBlock(   dim, dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)))            # shape=(4,64,128,256)
        self.down_sample_flow.append( nn.Sequential(
                                    Conv2dBlock(  dim, 2*dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn),            # shape=(4,128,64,128)
                                    Conv2dBlock(2*dim, 2*dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)))           # shape=(4,128,64,128)
        self.down_sample_flow.append(nn.Sequential(
                                    Conv2dBlock(2*dim, 4*dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn),            # shape=(4,256,32,64)
                                    Conv2dBlock(4*dim, 4*dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)))           # shape=(4,256,32,64)
        self.down_sample_flow.append(nn.Sequential(
                                    Conv2dBlock(4*dim, 8*dim, 4, 2, 1, norm, activ, pad_type, use_sn=use_sn),            # shape=(4,512,16,32)
                                    Conv2dBlock(8*dim, 8*dim, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn)))           # shape=(4,512,16,32)
        dim = 8*dim #dim=512

        # content decoder
        self.up_sample_flow.append(nn.Sequential(
                                    ResBlocks(n_res, dim, norm, activ, pad_type=pad_type),
                                    TransConv2dBlock(dim, dim//2, 6, 2, 2, norm=norm, activation=activ) ))  # kernel_size=6,,padding=2,stride=2 shape=(4,512,32,64)

        self.up_sample_flow.append(nn.Sequential(
                                    Conv2dBlock(dim, dim//2, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn),  # input_dim=512,kernel_size=5,output_dim=256,padding=2,strides=1,shape=(4,256,32,64)
                                    ResBlocks(n_res, dim//2, norm, activ, pad_type=pad_type),                 # shape=(4,256,32,64)
                                    TransConv2dBlock(dim//2, dim//4, 6, 2, 2, norm=norm, activation=activ) )) # shape=(4,128,32,64)

        self.location = nn.Sequential(
                                    Conv2dBlock(dim//2, dim//8, 5, 1, 2, norm, activ, pad_type, use_sn=use_sn),#shape=(4,64,16,32)
                                    Conv2dBlock(dim//8, 2, 3, 1, 1, norm='none', activation='none', pad_type=pad_type, use_bias=False) ) #shape=(4,2,16,32)
...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
        ...
        # initialize padding
        if pad_type == 'reflect':
            self.pad = nn.ReflectionPad2d(padding) #（laft, right, top, bottom） 输出大小的计算方式如下：
        ...
        norm_dim = output_dim #norm_dim1=64 norm_dim2=128 norm_dim3=256 output_dim1=64 output_dim2=128 output_dim3=256 output_dim4=512
        if norm == 'bn':
